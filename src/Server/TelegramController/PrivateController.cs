...
using ApiResponse = Rayshift.Models.ApiResponse;
...
public class PrivateController(
    ...
    public async Task SetupRayshift() {
        ...
        try {
            ...
            await ReplyTextMessageAsync("Errore nell'impostare Rayshift.io come provider.\n" +
                                        "Inviami lo screen dei tuoi support, /rayshift se vuoi riprovare la connessione a Rayshift.io o /skip se vuoi saltare questa fase");
            logger.LogError(e, "Error while requesting support lookup of {FriendCode}", TelegramChat["friend_code"]);
        }
    ...
    private async Task<List<RegisteredChatSettings>> GetRegisteredChatWithSettings(Master master) {
        // TODO Temporary fix, should directly fix data model
        try {
            ...
            Console.WriteLine(e);
            return new List<RegisteredChatSettings>();
        }
    ...
    private IReplyMarkup BuildSettingsKeyboard(Master master) {
        ...
        InlineKeyboardButton toggleRayshift;
        if (master.UseRayshift) {
            toggleRayshift = new InlineKeyboardButton {
                Text = "Disabilita Rayshift",
                CallbackData = new InlineDataWrapper(InlineKeyboardCommands.DisableRayshift,
                    data).ToString()
            };
        }
        else {
            toggleRayshift = new InlineKeyboardButton {
                ...
                CallbackData = new InlineDataWrapper(InlineKeyboardCommands.EnableRayshift,
                    data).ToString()
            };
        }
